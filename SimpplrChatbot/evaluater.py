...
class DeepEvaluator:
    def __init__(
        self, es_host: str, collection_name: str, num_eval_questions: int = 25
    ):
    ...
    def evaluate(self):
        ...
        for question in tqdm(self.k_eval_questions):
            ...
            test_case = LLMTestCase(
                ...
                retrieval_context=[text for text in rag_output["sources"]],
            )
            test_cases.append(test_case)
        evaluate(
            test_cases, [self.faithfulness_evaluator, self.answerrelevancy_evaluator]
        )
...
if __name__ == "__main__":
    ...
    evaluator = DeepEvaluator(
        "http://localhost:9200", os.environ.get("INDEX_NAME", "simpplr-chatbot"), 30
    )
    evaluator.evaluate()
